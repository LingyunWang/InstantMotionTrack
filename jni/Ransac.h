...
//typedef std::set<int *, SubSetIndexComparator> SubSet;
//typedef std::set<int *, SubSetIndexComparator>::iterator SubSetIter;
												    ...
												    double maximalOutlierPercentage)
{
	...
	for(i=0; i<numTries; i++) {
		...
		/*std::pair<std::set<int *, SubSetIndexComparator>::iterator, bool>*/
		auto res = chosenSubSets.insert(curSubSetIndexes);
		...
		if(res.second == true) { //first time we chose this sub set
	     ...
	     //release the memory
	/*std::set<int *, SubSetIndexComparator>::iterator*/auto it = chosenSubSets.begin();
	/*std::set<int *, SubSetIndexComparator>::iterator*/auto chosenSubSetsEnd = chosenSubSets.end();
